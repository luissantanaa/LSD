...
//   cc -Wall -march=native -O2 pseudo_random_generator.c -o pseudo_random_generator
// Run it as follows
//   ./pseudo_random_generator -v1 [n] [seed]     # this produces a v1 architecture with n internal bits
//   ./pseudo_random_generator -v2 [n] [seed]     # this produces a v2 architecture with n internal bits
// (defaults: n = 48, seed = current time)
//
// Due to the Cayley-Hamilton theorem the v2 generators are equivalent to linear feedback shift register generators
...
//
// program configuration (fan-ins and fan-outs for the v2 architecture)
//

#define min_n       48u  // smallest possible number of internal bits of the pseudo random number generator
#define max_n       64u  // largest possible number of internal bits of the pseudo random number generator
...
static u64 period; // the largest possible period of the v2 pseudo-random sequence
static u64 f[16];  // the factors of the largest possible period of the v2 pseudo-random sequence
static u32 nf;     // the number of distinct prime factors of the largest possible period of the v2 pseudo-random sequence
...
//
// v1 generator (counter + multi-stage substitution-permutation network based on big P-boxes and many small S-boxes)
//
// strong features: (hopefully) unpredictable, Hamiltonian path
// weak features: many logic cells, possible routing problems
//

static void output_v1_vhdl(void)
{
  ...
  {
    fprintf(stderr,"output_v1_vhdl(): illegal n (%u)\n",n);
    exit(1);
  }
  //
  // initializations
  ...
  printf("--\n");
  printf("architecture v1 of pseudo_random_generator is\n");
...
retry:  // scramble S-box (one to one map)
        ...
        // make sure the S-box is nonlinear
        for(i = 0u;i < 16u;i++)
        {
          ...
          if(j == 16u)
          { // S-box is linear
            fprintf(stderr,"S-box not good enough (linear)\n");
            goto retry; // Holly shit! A goto statement!
          }
        }
      }
  ...
  if(k < n)
  {
    fprintf(stderr,"output_v1_vhdl(): full avalanche effect not achieved!\n");
    exit(1);
  }
  printf("  --\n");
  printf("  -- State update\n");
...
//
// v2 generator (warped linear feedback shift register pseudo-random number generator)
//
...
static void find_v2_generator(u64 M[max_n])
{ // M = generator
...
static void output_v2_vhdl(u64 M[max_n])
{
  u32 i,j,k,f;
  ...
  printf("--\n");
  printf("architecture v2 of pseudo_random_generator is\n");
  ...
  if(f == 0u)
  {
    fprintf(stderr,"output_v2_vhdl(): kick signal not used!\n");
    exit(1);
  }
  printf("end v2;\n");
}

static void test_v2_generator(void)
{
# define test_n         24u
# define xor            ^
...
# define kick           0u
  ...
  if(bitmap == NULL)
  {
    fprintf(stderr,"test_v2_generator(): out of memory\n");
    exit(1);
  }
  n = test_n;
  ...
  for(;;)
  {
    ...
    //   ./pseudo_random_generator -v2 24 1
  ...
  if(c != (1u << n) - 1u)
  {
    fprintf(stderr,"test_v2_generator(): unexpected period (%08X)\n",c);
    exit(1);
  }
  free(bitmap);
# undef test_n
...
int main(int argc,char **argv)
{
  ...
  if(argc == 2 && argv[1][0] == '-' && argv[1][1] == 't' && argv[1][2] == 'm')
  {
    test_matrix_operations();
    fprintf(stderr,"All is well!\n");
    return 0;
  }
  if(argc == 2 && argv[1][0] == '-' && argv[1][1] == 't' && argv[1][2] == '2')
  {
    test_v2_generator();
    fprintf(stderr,"All is well!\n");
    return 0;
  }
  if(argc >= 2 && argv[1][0] == '-' && argv[1][1] == 'v' && argv[1][2] == '1')
  {
    n = 0u;
    seed = 0u;
    if(argc >= 3)
  ...
  if(argc >= 2 && argv[1][0] == '-' && argv[1][1] == 'v' && argv[1][2] == '2')
  {
    n = 0u;
    seed = 0u;
    if(argc >= 3)
  ...
  fprintf(stderr,"usage: pseudo_random_generator -v1 [n] [seed]  # VHDL code for the v1 generator\n");
  fprintf(stderr,"       pseudo_random_generator -v2 [n] [seed]  # VHDL code for the v2 generator\n");
  fprintf(stderr,"       pseudo_random_generator -tm | gp -q     # test matrix operations\n");
  fprintf(stderr,"       pseudo_random_generator -t1             # test v1 generator\n");
  fprintf(stderr,"       pseudo_random_generator -t2             # test v2 generator\n");
  fprintf(stderr,"by default n is 48\n");
  fprintf(stderr,"by default seed is the current time\n");
  return 0;
}
